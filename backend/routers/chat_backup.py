                    ...
                    if tool_data['name'] in ['generateImage', 'createDiagram']:
                        try:
                            ...
                            try:
                                ...
                                transition_response = {
                                    'type': 'content',
                                    'content': f"🎵 Creating your audio now...\n\n",
                                    'content_type': 'text',
                                    'provider': 'openai',
                                    'model': actual_model,
                                    'multimodal_content': [{
                                        'type': 'text',
                                        'data': f"🎵 Creating your audio content...\n\n",
                                        'format': 'text'
                                    }],
                                    'preserve_formatting': True
                                }
                            ...
                            try:
                                ...
                                transition_response = {
                                    'type': 'content',
                                    'content': f"🎤 Transcribing your audio now...\n\n",
                                    'content_type': 'text',
                                    'provider': 'openai',
                                    'model': actual_model,
                                    'multimodal_content': [{
                                        'type': 'text',
                                        'data': f"🎤 Processing your audio content...\n\n",
                                        'format': 'text'
                                    }],
                                    'preserve_formatting': True
                                }
                            ...
                            try:
                                ...
                                transition_response = {
                                    'type': 'content',
                                    'content': f"📄 Processing your file now...\n\n",
                                    'content_type': 'text',
                                    'provider': 'openai',
                                    'model': actual_model,
                                    'multimodal_content': [{
                                        'type': 'text',
                                        'data': f"📄 Analyzing your file content...\n\n",
                                        'format': 'text'
                                    }],
                                    'preserve_formatting': True
                                }
...
async def execute_audio_generation_tool(tool_name: str, tool_args: dict, provider: str):
    ...
    try:
        ...
        voice = tool_args.get('voice', 'alloy')  # alloy, echo, fable, onyx, nova, shimmer
        model = tool_args.get('model', 'tts-1')  # tts-1 or tts-1-hd
        ...
        response = service.client.audio.speech.create(
            model=model,
            voice=voice,
            input=text,
            speed=speed
        ...
        # For now, return a data URL (in production, you'd save to storage)
        # Convert response to base64 for embedding
        import base64
        ...
        return {
            "success": True,
            "audio_url": audio_url,
            "url": audio_url,  # Duplicate for compatibility
            "text": text,
            "voice": voice,
            "model": model,
            "speed": speed,
            "provider": "openai",  # Always OpenAI for audio
            "original_provider": provider,  # Track original requesting provider
            "tool_name": tool_name,
        ...
        return {
            ...
            "voice": voice if 'voice' in locals() else 'alloy',
            "provider": "openai",
...
async def execute_speech_to_text_tool(tool_name: str, tool_args: dict, provider: str):
    ...
    try:
        ...
        audio_data = tool_args.get('audio_data')  # base64 encoded audio
        language = tool_args.get('language', 'auto')  # auto-detect or specific language
        ...
        if audio_data:
            ...
            audio_file_obj.name = "audio.mp3"  # Whisper needs a filename
        else:
...
async def execute_file_processing_tool(tool_name: str, tool_args: dict, provider: str):
    ...
    try:
        ...
        task = tool_args.get('task', 'summarize')  # summarize, analyze, extract
        ...
        if task == 'summarize':
            instruction = "Provide a concise summary of this content, highlighting the key points and main ideas."
        elif task == 'analyze':
            instruction = "Analyze this content in detail, providing insights, patterns, and important observations."
        elif task == 'extract':
            instruction = "Extract the most important information, data points, and actionable items from this content."
        else:
            instruction = f"Process this content with the following task: {task}"
        
        # Use the current provider to process the file
